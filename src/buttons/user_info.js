...
export default async function userInfo(interaction) {
  ...
  if (!isStaff(interaction.member, config)) return interaction.reply({ content: '❌ Only staff can view user info.', ephemeral: true });

  const member = await interaction.guild.members.fetch(ticket.user_id).catch(() => null);
  if (!member) return interaction.reply({ content: '❌ Ticket owner not found — they may have left the server.', ephemeral: true });
